...
namespace CharacterCustomizationTool.Editor
{
    public class CharacterCustomizationWindow : EditorWindow
    {
        ...
        {
            // Release cached objects so they are recreated cleanly on next open
            ...
            if (_camera != null)
            {
                DestroyImmediate(_camera.transform.parent.gameObject);
                _camera = null;
            }
        }
        ...
        private void InitializeCamera()
        {
            ...
            // URP forces its own MSAA from the URP Asset, overriding RenderTexture.antiAliasing.
            // We must disable it explicitly via UniversalAdditionalCameraData to prevent
            // "Missing resolve surface" / "Not inside a Renderpass" errors in EditorWindows.
            var urpCameraData = cameraObject.GetComponent<UniversalAdditionalCameraData>()
        ...
        private void CreateRenderTexture()
        {
            ...
            _renderTexture = new RenderTexture(300, 300, 24, RenderTextureFormat.ARGB32)
            {
                antiAliasing = 1  // Must be 1 for URP: MSAA resolve is unsupported when Camera.Render()
                                  // is called manually inside an EditorWindow (no backbuffer resolve surface).
            };
        }
